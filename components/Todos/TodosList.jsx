    ...
    return ( 
        ...
        border-gray-100 mb-4 p-3 md:p-4 rounded-xl">
            <Link href={`/todos/${todo._id}`}>
              <span className={`${todo.isCompeleted ? "line-through":""}`}>{todo.title}</span>
            </Link>
          
          <div className="flex gap-x-3 items-center">
            <button className="" onClick={()=>onComplete(todo._id)}>
           {todo.isCompeleted ?  <svg className="w-6 h-6 stroke-green-400" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
